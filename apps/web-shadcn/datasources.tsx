...
// @ts-expect-error - shared module
import { reports } from '@ng-analytics/shared';

const FREQ_ORDER: Record<string,number> = { Daily:0, Weekly:1, Monthly:2, Annual:3 };
...
function FreqBadge({ freq }: { freq: string }) {
  ...
  return <span style={{display:'inline-block',padding:'2px 8px',borderRadius:4,fontSize:11,fontWeight:700,letterSpacing:'0.06em',fontFamily:"'IBM Plex Mono',monospace",background:c.bg,color:c.text,border:`1px solid ${c.border}`,whiteSpace:'nowrap'}}>{freq.toUpperCase()}</span>;
}
...
function EIATable({ searchQ }: { searchQ: string }) {
  ...
  const rows = useMemo(() => {
    ...
    if (searchQ) {
      ...
      d = d.filter((r: any) => r.name.toLowerCase().includes(q) || r.acronym.toLowerCase().includes(q) || r.dataTypes.toLowerCase().includes(q) || r.coverage.toLowerCase().includes(q));
    }
    d.sort((a: any, b: any) => {
    ...
    <div style={{borderRadius:10,border:'1px solid rgba(255,255,255,0.08)',overflow:'hidden',boxShadow:'0 4px 32px rgba(0,0,0,0.5)'}}>
      <div style={{overflowX:'auto'}}>
        <table style={{width:'100%',borderCollapse:'collapse',tableLayout:'fixed',minWidth:900}}>
          <colgroup>{EIA_COLS.map(c=><col key={c.key} style={{width:c.width}}/>)}</colgroup>
          <thead>
      ...
      <div style={{background:'#13161e',borderTop:'1px solid rgba(255,255,255,0.06)',padding:'10px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}>
      ...
      <div className="mx-auto max-w-[1400px] p-6 lg:p-10">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-3 mt-5">
            ...
            {(Object.entries(CAT_CLS) as [string,string][]).filter(([c])=>c!=='EIA Reports').map(([cat,cls])=>(
              ...
              {filteredOther.map((ds,idx)=>(
                <div key={ds.name} className={`grid grid-cols-[1.8fr_1.2fr_1fr_0.85fr] gap-x-4 px-6 py-5 border-b border-white/[0.04] hover:bg-white/[0.02] last:border-0 ${idx%2!==0?'bg-white/[0.01]':''}`}>
                  <div className="flex flex-col gap-2 min-w-0">
                    ...
                    <span className={`w-fit text-[10px] font-bold px-2 py-0.5 rounded-full border ${CAT_CLS[ds.category]??''}`}>{ds.category}</span>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    <span className={`w-fit text-[10px] font-bold px-2 py-1 rounded-full border ${ACCESS_CLS[ds.access]??'text-[#6b7280] bg-white/[0.05] border-white/10'}`}>{ds.access}</span>
                    <span className={`w-fit text-[10px] font-bold px-2 py-1 rounded-full border ${FREQ_BADGE[ds.frequency]??'text-[#6b7280] bg-white/[0.04] border-white/10'}`}>{ds.frequency}</span>
