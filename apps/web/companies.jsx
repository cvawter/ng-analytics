...
const MODAL_TABS = ["Overview", "Stock Chart", "Options Chain", "Regulatory"];

function StockChart({ ticker, history }) {
            ...
            <div className="h-48 w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={history} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
                        ...
                        <YAxis domain={[minP * 0.97, maxP * 1.03]} tick={{ fill: "#6b7280", fontSize: 11 }} tickLine={false} axisLine={false} tickFormatter={v => `$${v.toFixed(0)}`} width={50} />
                        <Tooltip content={<CustomTooltip />} />
                        <ReferenceLine y={first} stroke="rgba(255,255,255,0.1)" strokeDasharray="4 2" />
                        <Area type="monotone" dataKey="price" stroke={isUp ? "#10b981" : "#ef4444"} strokeWidth={2} fill="url(#chartGrad)" dot={false} />
                    </AreaChart>
                </ResponsiveContainer>
            </div>
        </div>
    ...
    return (
        <div>
            <div className="flex gap-2 mb-4">
                {chain.map((c, i) => (
                    <button key={c.expiration} onClick={() => setSelectedExp(i)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition-all ${i === selectedExp ? "bg-[#f0a500]/20 border border-[#f0a500]/40 text-[#f0a500]" : "bg-white/[0.04] border border-white/10 text-[#8a94a6] hover:text-[#eef0f4]"}`}>
                        {c.expiration}
                    </button>
                ))}
            </div>
                ...
                <table className="w-full text-xs">
                    ...
                    <tbody>
                        {strikes.map(({ strike, call, put }) => {
                            ...
                            return (
                                <tr key={strike} className={`border-b border-white/[0.04] transition-colors ${isATM ? "bg-[#f0a500]/5 border-[#f0a500]/20" : "hover:bg-white/[0.02]"}`}>
                                    <td className="py-2 px-2 text-emerald-400 font-mono">{call.bid.toFixed(2)}</td>
                                    <td className="py-2 px-2 text-emerald-400/70 font-mono">{call.ask.toFixed(2)}</td>
                                    <td className="py-2 px-2 text-[#8a94a6] font-mono">{call.iv}%</td>
                                    <td className={`py-2 px-3 text-center font-bold font-mono ${isATM ? "text-[#f0a500]" : "text-[#eef0f4]"}`}>
                                        ${strike}{isATM && <span className="ml-1 text-[9px] bg-[#f0a500]/20 text-[#f0a500] rounded px-1">ATM</span>}
                                    </td>
                                    <td className="py-2 px-2 text-right text-[#8a94a6] font-mono">{put.iv}%</td>
                                    <td className="py-2 px-2 text-right text-red-400 font-mono">{put.bid.toFixed(2)}</td>
                                    <td className="py-2 px-2 text-right text-red-400/70 font-mono">{put.ask.toFixed(2)}</td>
                                </tr>
                            );
                        })}
                    </tbody>
...
function CompanyModal({ company, onClose }) {
    ...
    const basePrice = TICKER_BASE_PRICES[company.ticker] || 50;
        ...
        <>
            ...
            <motion.div initial={{ opacity: 0, y: 40, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                ...
                <div className="overflow-y-auto flex-1 p-6">
                    ...
                    {tab === "Regulatory" && company.regulatoryActivity?.length > 0 && (
                        <div className="flex flex-col gap-3">
                            ...
                            {company.regulatoryActivity.map((docket, idx) => (
                                                            ...
                                                            <div className="flex flex-col gap-2">
                                                                {docket.documents.map((doc, dIdx) => (
                                                                    <a key={dIdx} href={doc.url} target="_blank" rel="noreferrer" className="flex items-center justify-between p-2 rounded-lg bg-white/[0.03] hover:bg-white/[0.06] border border-white/5 transition-colors group">
                                                                        <div className="flex items-center gap-2"><FileText className="h-4 w-4 text-[#8a94a6] group-hover:text-[#f0a500] transition-colors" /><span className="text-[13px] text-[#c8cdd8] group-hover:text-white transition-colors">{doc.title}</span></div>
                    ...
                    {tab === "Regulatory" && (!company.regulatoryActivity || company.regulatoryActivity.length === 0) && (
...
export default function Companies() {
    ...
    const types = ["All", ...new Set(energyDemandCompanies.map(c => c.type))];
    const filtered = energyDemandCompanies.filter(company => {
                    ...
                    <div className="flex flex-wrap gap-2">
                        {types.map(t => (
                            <button key={t} onClick={() => setFilterType(t)}
                                className={`rounded-md px-3 py-1.5 font-mono text-[11px] font-bold uppercase tracking-wider transition-all ${filterType === t ? "bg-[#f0a500]/15 border border-[#f0a500]/40 text-[#f0a500]" : "border border-white/[0.08] bg-white/[0.03] text-[#6b7280] hover:text-[#eef0f4]"}`}>
                                {t}
                            </button>
                        ))}
                    </div>
