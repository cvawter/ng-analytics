...
const COMMODITIES = [
  ...
  },
  {
    ...
    icon: Droplets,
    spot: oilSpotPrices,
    history: oilSpotHistory,
    futures: oilFuturesCurve,
    accentColor: '#60a5fa',
    gradientId: 'oilGrad',
  },
  {
    ...
    label: 'Silver',
    icon: Gem,
    spot: silverSpotPrices,
    history: silverSpotHistory,
    futures: silverFuturesCurve,
    accentColor: '#a78bfa',
  ...
  },
  {
    ...
    icon: Zap,
    spot: reeSpotPrices,
    history: reeSpotHistory,
    futures: reeFuturesCurve,
    accentColor: '#34d399',
    gradientId: 'reeGrad',
  },
  {
    ...
    icon: Battery,
    spot: batteryMetalsSpot,
    history: batteryMetalsHistory,
    futures: batteryMetalsFutures,
    accentColor: '#f472b6',
...
function SpotTooltip({ active, payload, unit }) {
...
function FuturesToolTip({ active, payload, unit }) {
  if (!active || !payload?.length) return null;
  const p = payload[0];
  return (
    <div className="bg-[#161922] border border-white/10 rounded-xl px-3 py-2 shadow-xl text-sm">
...
function CommodityView({ commodity }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        {/* Stats grid — hide for REE (uses individual panel instead) */}
        ...
        {(commodity.key === 'ree' || commodity.key === 'battery') && (
          <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-3 content-start">
            {(commodity.key === 'ree' ? reeIndividualPrices : batteryMetalsIndividual).map(r => (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={history} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
              ...
              <YAxis domain={[minSpot * 0.97, maxSpot * 1.03]} tick={{ fill: '#9ca3af', fontSize: 11 }} tickLine={false} axisLine={false}
                tickFormatter={v => `$${v.toFixed(decimals === 3 ? 2 : 0)}`} width={58} />
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={futures} margin={{ top: 16, right: 28, left: 16, bottom: 4 }}>
              ...
              <YAxis domain={[minFut, maxFut]} tick={{ fill: '#9ca3af', fontSize: 12 }} tickLine={false} axisLine={false}
                tickFormatter={v => `$${v.toFixed(2)}`} dx={-10} />
  ...
  return (
    <div className="min-h-screen bg-[#0d0f14] text-[#eef0f4] p-6 lg:p-10 font-sans overflow-y-auto">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* Active Commodity View */}
        {activeCommodity && <CommodityView commodity={activeCommodity} />}
